import {autoRegistrarVigilante} from '../fetchs/vigilantes-fetch.js';
...
function eventoAutoRegistrarVigilante(){
    const formularioVigilante = document.getElementById('formulario_vigilante');
    formularioVigilante.addEventListener('submit', (e)=>{
        e.preventDefault();
        let formData = new FormData(formularioVigilante);
        formData.append('operacion', 'auto_registrar_vigilante');

        autoRegistrarVigilante(formData, urlBase).then(respuesta=>{
...
function alertaExito(respuesta){
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.replace(urlBase+'auto-registro-vigilantes');
        } 
    });
}
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    botonRegistrar = document.getElementById('btn_registrarme_vigilante');

    motrarCampos();
    volverCampos();
    eventoAutoRegistrarVigilante();
})
